function [Sr] = dynaRespFD3(Bridge,Wind,indR,varargin)
...
% *************************************************************************
    ...
    function [cohU,cohV,cohW] = VonKarmanCoherence(Lu,D,f,U)
        % function [cohU,cohV,cohW] = VonKarmanCoherence(Lu,D,f,U)
        % Calculate the coherence according to von Karman
        %
        %% INPUTS
        %
        % Lu: scalar: along-wind integrale length scale
        % D: scalar:  spatial separation
        % f: scalar:  frequency
        % U: scalar:  mean wind speed
        %% OUTPUTS
        %
        %  cohU: vector: root-coherence for u-component
        %  cohV: vector: root-coherence for v-component
        %  cohW: vector: root-coherence for w-component
        % Turbulence length scale
        L = 2.*Lu;
        % reduced frequency
        n = 2*pi.*f.*D./U;
        
        eta = sqrt(n.^2+(0.747*D/L).^2);
        % along wind component u
        A0 = eta.^(5/6).*besselk(5/6,eta);
        B0 = -1/2.*eta.^(11/6).*besselk(1/6,eta);
        cohU = (2^(1/6)/gamma(5/6).*(A0+B0)) ;
        cohU(isnan(cohU))=1;
        % cross wind component v
        
        r = D./L;
        B1 = 0.597./(2.869.*eta.^2./r^2-1);
        B2 = ((4.781.*eta.^2.*A0./r.^2) - eta.^(11/6).*besselk(11/6,eta));
        cohV = abs(B1.*B2);
        cohV(isnan(cohV))=1;
        
        % cross wind component w
        a = gamma(1/3)./(sqrt(pi)*gamma(5/6));
        B0 = -3*(D./(a*L)).^2./(3*eta.^2+5.*(2*pi.*f.*D./U).^2).*eta.^(11/6).*besselk(1/6,eta);
        cohW = (2^(1/6)/gamma(5/6).*(A0+B0)) ;
        cohW(isnan(cohW))=1;
    end
